...
job <- read_tsv(paste(data_dir, "job.txt", sep="/")) %>%
  select(-Host, -Receive, -Send) %>%
  arrange(Seq)
results <- read_csv(paste(data_dir, "results.csv", sep="/")) %>%
  mutate(problem = str_sub(problem, 11),
         model = "hacker-toolkit") %>%
  select(model, problem, run, trial, accuracy, everything())
import_other_models <- function() {
  other_results_1_5 <-
    read_csv("../../list-routine-model-comparison/waves/1_5/data/results.csv") %>%
    mutate(concept = str_c("1.", concept))
  other_results_2 <-
    read_csv("../../list-routine-model-comparison/waves/2/data/results.csv") %>%
    mutate(concept = str_c("2.", concept))
  bind_rows(other_results_1_5, other_results_2) %>%
    rename(problem = concept) %>%
    filter(trial <= 11) %>%
    select(model, problem, run, trial, accuracy)
}
other_results <- import_other_models()
```
...
```{r plot-n-seen, echo=F, warning=F, out.width="100%", fig.align='center', fig.show='hold', fig.height=10.5, fig.width=14, fig.cap="Number of programs (y-axis) considered per trial (x-axis)."}
plot_n_seen <- function(data) {
  data <- data %>% arrange(problem, run, trial)
  ggplot(data, aes(x = trial, y = n_seen, fill=run)) +
    geom_bar(stat = "identity") +
    ylab("Number of programs seen") +
    xlab("Trial") +
    facet_wrap(~problem) +
    theme(legend.position = "none")
}
...
plot_how_many <- function(data) {
  data <- data %>% group_by(k_learned) %>% count()
  ggplot(data, aes(x=k_learned, y=n)) +
    geom_bar(stat='identity') +
...
plot_how_many_per_problem <- function(data) {
  ggplot(data, aes(x=problem, y=k_learned, fill=problem)) +
    geom_bar(stat='identity') +
    xlab("Problem") +
    ylab("Number of runs") +
    theme(legend.position = "none") +
...
```{r model-comparison, cache=T, echo=F, warning=F}
model_comparison_results <- results %>%
  select(-program, -n_seen) %>%
  bind_rows(other_results)
model_comparison_accuracy_overall <- model_comparison_results %>%
  group_by(model, problem) %>%
  tidyboot_mean(column = accuracy, nboot = 500)
model_comparison_how_many <- make_how_many(model_comparison_results)
```


```{r plot-model-comparison-accuracy-overall, dependson=c("model-comparison"), echo=F, warning=F, fig.align='center', fig.width=21, fig.height=14, fig.cap="Probability of correct response (y-axis) by concept (x-axis) by model (subplots). Errors bars are bootstrapped 95% CI."}
plot_model_comparison_accuracy_overall <- function(data) {
  ggplot(data, aes(x=problem, y=empirical_stat, fill=problem)) +
    geom_bar(stat='identity') +
    geom_errorbar(aes(x=problem, ymin=ci_lower, ymax=ci_upper), width=0.5) +
    ylab("Mean Performance") +
    xlab("Problem") +
...
plot_model_comparison_accuracy_overall(model_comparison_accuracy_overall)
```

```{r plotmodel-comparison-how-many-per-problem, dependson=c("model-comparison"), echo=F, warning=F, fig.align='center', fig.height=14, fig.width=21, fig.show='hold', out.width="100%", fig.cap="Number of runs (y-axis) for which the model solved each problem (x-axis) by model (subplots). A problem is solved after 3 consecutive correct responses."}
plot_model_comparison_how_many_per_problem <- function(data) {
  data <- data %>% mutate(mean_learned = k_learned/n_learned)
  ggplot(data, aes(x=problem, y=mean_learned, fill=problem)) +
    geom_bar(stat='identity') +
    xlab("Problem") +
    ylab("Number of runs") +
    facet_wrap(~model, ncol = 1) +
...
plot_model_comparison_how_many_per_problem(model_comparison_how_many)
```
