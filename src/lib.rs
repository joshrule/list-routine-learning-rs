...
pub fn search_online<'ctx, 'b, R: Rng>(
    ...
    for (i, chain) in chain1.pool.iter_mut().enumerate() {
        ...
        let score = -chain.1.current().at_temperature(Temperature::new(4.0, 1.0));
        reservoir.add(
    ...
    for n_data in 0..n_trials {
        ...
        best = -h_initial.at_temperature(Temperature::new(4.0, 1.0));
        writeln!(
            best_file,
            "{}",
...
pub fn search_batch<'ctx, 'b, R: Rng>(
    ...
    let h_initial = &top_n.least().unwrap().data.0;
    let mut best = -h_initial.at_temperature(Temperature::new(4.0, 1.0));
    writeln!(
        best_file,
        "{}",
        hypothesis_string_mcmc(problem, order, run, n_data, h_initial, true, None)
